...
OUTPUT_FILE = Path("hot_bootstrap_comparisons_with_means.txt")
...
def bootstrap_overhead_mean(without: list[float], with_otel: list[float], n_iter: int = 100000) -> np.ndarray:
    ohne_arr, mit_arr = np.array(without), np.array(with_otel)
    n_ohne, n_mit = len(ohne_arr), len(mit_arr)
    overheads = []
    for _ in range(n_iter):
        mean_ohne = np.mean(np.random.choice(ohne_arr, size=n_ohne, replace=True))
        mean_mit = np.mean(np.random.choice(mit_arr, size=n_mit, replace=True))
        overheads.append((mean_mit - mean_ohne) / mean_ohne)
...
def main():
    ...
    results = ["=== BOOTSTRAP MEAN ANALYSE (HOT STARTS) ===\n"]
            ...
            for bench in all_bench:
                ...
                if "without_otel" in f_pair and "with_otel" in f_pair:
                    ...
                    if len(v_wo) > 1 and len(v_wi) > 1:
                        ovs = bootstrap_overhead_mean(v_wo, v_wi)
                        ov_data[(lang, bench)] = ovs
                        summaries.append(f"{lang.upper():7} | {bench:15} | Ohne: {np.mean(v_wo):7.2f} | Mit: {np.mean(v_wi):7.2f} | OH: {np.mean(ovs):7.2%}")
