...
OUTPUT_FILE = Path("cold_bootstrap_comparisons_with_medians.txt")
...
def parse_metrics(csv_path: Path) -> dict[str, list[float]]:
    """Extrahiert Metriken aus der @message Spalte der CSV-Dateien."""
    metrics = {
        "duration_ms": [],
        "init_duration_ms": [],
        "max_memory_mb": [],
...
def bootstrap_overhead_median(without: list[float], with_otel: list[float], n_iter: int = 100000) -> np.ndarray:
    """
    Erzeugt n_iter Bootstrap-Schätzungen des prozentualen Overheads basierend auf MEDIANEN.
    overhead = (median_mit - median_ohne) / median_ohne
    """
    ...
    for _ in range(n_iter):
        ...
        med_ohne = np.median(boot_ohne)
        med_mit = np.median(boot_mit)
        
        overhead = (med_mit - med_ohne) / med_ohne
...
def main() -> None:
    ...
    results_lines.append("BOOTSTRAP-ANALYSE: RELATIVER OVERHEAD AUF MEDIAN-BASIS")
    results_lines.append("Datentyp: COLD STARTS")
    ...
    for metric_key in metric_keys:
        ...
        for language in sorted(LANGUAGES):
            for benchmark in all_benchmarks:
                ...
                real_med_without = np.median(without_vals)
                real_med_with = np.median(with_vals)

                overheads = bootstrap_overhead_median(without_vals, with_vals, n_iter=100000)
                ...
                language_summaries.append(
                    ...
                    f"Med-Ohne: {real_med_without:7.2f}{unit} | Med-Mit: {real_med_with:7.2f}{unit} | "
                    f"Overhead: {mean_oh:7.2%} (std={std_oh:.4f})"
                )

        results_lines.append("\n[EINZELWERTE ÜBERSICHT (MEDIAN)]")
        results_lines.extend(sorted(language_summaries))
        results_lines.append("")

        results_lines.append("[STATISTISCHE PAARVERGLEICHE DER MEDIAN-OVERHEADS]")
    ...
    print(f"Median-Analyse abgeschlossen. Ergebnisse geschrieben in: {OUTPUT_FILE}")
